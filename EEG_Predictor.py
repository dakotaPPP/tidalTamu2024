...
optimal_epochs = [9, 5, 10, 20, 4, 16, 21, 50, 18, 66, 30, 34]
def train_pytorch_model(model, criterion, optimizer, X_train, y_train, X_val, y_val, model_name, disorder_index):
    num_epochs = optimal_epochs[disorder_index]  # Increase number of epochs
    batch_size = 32
    patience = 50  # Patience for early stopping
    trigger_times = 0
    best_val_f1 = 0  # Use F1 score instead of loss for early stopping. F1 score is more relevant here
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        x, _ = self.lstm(x)
        x = x[:, -1, :]  # Gets the last output of a sequence
...
# Training function for sklearn models (all that come from sklearn, which is just not LSTM and Transformer)
...
# Function to train and evaluate models against each other
def train_evaluate_model(model_name, disorder_index):
    ...
    if model_name == 'LSTM':
        ...
        train_loss, val_loss, train_f1, val_f1 = train_pytorch_model(model, criterion, optimizer, X_train_resampled, y_train_resampled, X_val_full, y_val, model_name, disorder_index)
    ...
    # For elastic net regression model
    elif model_name == 'EN':
    ...
    # Finally for all the other models
    else:
        test_preds = model.predict(X_test)
...
for disorder_index in range(num_classes):
    ...
    # For regression model (Elastic Net), use R2 score
...
# Train the final model. this one combines all the probabilities from the best models and just does a linear combination between them to make a final prediction
...
# Evaluate the final model and print the resulting accuracy
